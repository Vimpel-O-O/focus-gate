...
let provider = CommandLine.arguments.count > 2 ? CommandLine.arguments[2] : ""
let names = ["anthropic": "Anthropic (Claude)", "gemini": "Google (Gemini)", "xai": "xAI (Grok)"]
guard let name = names[provider] else { exit(2) }
let service = "com.focusgate." + provider
...
if command == "set" {
    ...
    if status == errSecItemNotFound {
        ...
        item[kSecAttrLabel as String] = "Focus Gate " + name + " API key"
